...
class Level:
    def __init__(self, map_path, scenario_path):
        ...
        for row, row_data in enumerate(grid):
            for col, field in enumerate(row_data.strip()):
                ...
                if row != 0 and grid[row-1][col] != "@":
                    self.g.add_edge(field_id, coords_to_id(self.cols, row-1, col))
                if row != self.rows - 1 and grid[row+1][col] != "@":
                    self.g.add_edge(field_id, coords_to_id(self.cols, row+1, col))
                if col != 0 and grid[row][col - 1] != "@":
                    self.g.add_edge(field_id, coords_to_id(self.cols, row, col - 1))
